...
// Admin side is always light — remove dark class if student left it on
...
const AdminLayout = () => {
    ...
    return (
        <div className="main-admin-layout flex flex-col md:flex-row h-screen bg-white">
            ...
            {/* ── Desktop Sidebar ── */}
            <div className="desktop-sidebar hidden md:flex w-56 p-4 flex-col shrink-0 bg-gray-100">
                ...
                <nav className="flex flex-col gap-1">
                    ...
                    <div>
                        ...
                        <div className={`overflow-hidden transition-all duration-200 ${studentsOpen ? "max-h-60 opacity-100 mt-1" : "max-h-0 opacity-0"}`}>
                            <div className="ml-3 pl-3 flex flex-col gap-0.5 border-l-2 border-gray-300">
                                {studentSubItems.map(sub => (
                                    <NavLink key={sub.path} to={sub.path}
                                        className={({ isActive }) =>
                                            ...
                                            ${isActive ? "bg-orange-500/15 text-orange-600 font-semibold" : "text-gray-500 hover:bg-gray-200 hover:text-gray-700"}`}>
                                        {sub.icon}
                                        <span>{sub.label}</span>
                                    </NavLink>
                                ))}
                    ...
                    {/* Settings — eso_officer only */}
                    {user?.role === "eso_officer" && (
            ...
            <div className="mobile-bottom-nav-container fixed bottom-0 left-0 right-0 md:hidden
                border-t shadow-md flex justify-around items-center h-16 z-50 bg-white border-gray-200">
                ...
                <div ref={dropUpRef} className="relative flex flex-col items-center">
                    <button
                        ...
                        className={`flex flex-col items-center justify-center gap-0.5 px-3 py-1
                            ${isStudentsPath || studentsDropUp ? "text-orange-500 font-bold" : "text-gray-500"}`}>
                    ...
                    {studentsDropUp && (
                        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2
                            ...
                            w-48 z-50 bg-white border border-gray-100">
                            <div className="px-3 py-2 text-[9px] font-bold uppercase tracking-widest text-gray-400 border-b border-gray-100">
                                Students
                            </div>
                            {studentSubItems.map(sub => (
                                <NavLink key={sub.path} to={sub.path}
            ...
            {showLogoutModal && (
                <div className="logout-modal fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50">
                    <div className="anim-slide-up rounded-xl shadow-[0_24px_64px_rgba(0,0,0,0.35)]
                        w-[90%] sm:w-[400px] p-6 relative bg-white text-gray-800">
